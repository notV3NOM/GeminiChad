...
def extract_artifacts(llm_reponse: str):
    ...
    if matches:
        ...
        for language, content in matches:
            extension = EXTENSION_MAPPING[language.lower()] if language.lower() in EXTENSION_MAPPING else "txt"
            temp_file_path = temp_file_path = f'data/temp-{str(uuid.uuid4())}.{extension}'
            with open(temp_file_path, 'wb') as f:
                f.write(content.strip().encode())
            artifacts.append(discord.File(temp_file_path, filename=f'temp-{str(uuid.uuid4())}.{extension}'))
            artifact_path_list.append(temp_file_path)
            llm_reponse = llm_reponse.replace(f"```{language}\n{content}\n```",  f"<<artifact_{artifacts_count}>>")
            artifacts_count+=1
...
def extract_images(llm_response: str):
    ...
    if image_matches:
        ...
        for image_match in image_matches:
            ...
            embeds.append(embed)
            files.append(image)
            llm_response = llm_response.replace(f'<IMAGE>{image_match}</IMAGE>', f"<<image_{images_count}>>")
            images_count+=1
...
def setup_event_handlers(client: discord.Client):
    ...
    async def on_message(message: discord.Message):
        ...
        mentions_bot_name = BOT_NAME.lower() in message.content.lower() or client.user.mention in message.content
        ...
        if message.attachments:
            for attachment in message.attachments:
                if attachment.content_type.startswith('image/') or attachment.filename.endswith(('.txt', '.py', '.js', '.tsx', '.pdf')):
                    resp = requests.get(attachment.url)
                    if resp.status_code == 200:
                        file_data = resp.content
                        temp_file_path = f'data/temp-{str(uuid.uuid4())}-{attachment.filename}'
        ...
        async with message.channel.typing(): 
            response = await asyncio.to_thread(chat, prompt, SESSIONS[message.channel.id][CHAT_SESSION], file_path_list)
