...
class ReminderManager:
    ...
    def add_reminder(self, user_id, message, time_str, channel_id):
        ...
        """
        reminder_time = dt.datetime.strptime(time_str.split('.')[0], "%Y-%m-%dT%H:%M:%S")
    ...
    async def check_reminders(self, client):
        ...
        due_reminders = [reminder for reminder in self.reminders if dt.datetime.fromisoformat(reminder["time"]).astimezone(dt.timezone.utc) <= now_utc]
        self.reminders = [reminder for reminder in self.reminders if dt.datetime.fromisoformat(reminder["time"]).astimezone(dt.timezone.utc) > now_utc]
        self.save_reminders()
        ...
        for reminder in due_reminders:
            try:
                ...
                response = chat(REMINDER_TEMPLATE.format(reminder=reminder['message']), temp_session())
